...
public class UserActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    Button BackBtn;
    EditText etName, etService, etResult, etDate, etPrice, search;
    Spinner filtrSpinner, sortSpinner;

    DBHelper dbHelper;
    SQLiteDatabase database;

    String name, fil, sor, sear;
    String[] filtr, sortir;
                ...
                if (fil.equals(getResources().getStringArray(R.array.filtr)[1])) {
                    if (sor != null) {
                        if (sor.equals(getResources().getStringArray(R.array.sortir)[1])) {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_RESULT + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, "+", sear}, null, null, DBHelper.KEY_DATE + " ASC");
                        } else if (sor.equals(getResources().getStringArray(R.array.sortir)[2])) {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_RESULT + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, "+", sear}, null, null, DBHelper.KEY_DATE + " DESC");
                        } else {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_RESULT + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, "+", sear}, null, null, null);
                        }
                    }
                    ...
                    if (sor != null) {
                        if (sor.equals(getResources().getStringArray(R.array.sortir)[1])) {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, sear}, null, null, DBHelper.KEY_DATE + " ASC");
                        } else if (sor.equals(getResources().getStringArray(R.array.sortir)[2])) {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, sear}, null, null, DBHelper.KEY_DATE + " DESC");
                        } else {
                            cursor = database.query(DBHelper.TABLE_RESULTS, null, DBHelper.KEY_NAME2 + " = ? AND " + DBHelper.KEY_SERVICE + " = ?", new String[]{name, sear}, null, null, null);
                        }
                    }
                }
            }
